...
prune_image() {
  if [[ -n $(docker images -q $BUILDER_CONTAINER) ]]; then
    log "Removing the builder image..."
...
prune_store() {
  if [[ -n $(docker ps -a | grep $DATA_CONTAINER) ]]; then
    log "Removing the builder container and nix store cache..."

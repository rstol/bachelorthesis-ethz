...
build_image() {
  ...
  if [[ -n $(docker images -q $BUILDER_CONTAINER) ]] && [[ -n $(docker ps -a | grep $DATA_CONTAINER) ]]; then
    ...
    log "Builder container or data container not found. Try building them first."
    exit 1
  fi
}
...
while test $# -gt 0; do
  ...
  run)
    ...
    exit 0
    ;;
  build-streamed-startup)
    docker run --rm --volumes-from=$DATA_CONTAINER -v $PROJECTROOT:/home/user --add-host host.docker.internal:host-gateway --workdir="/home/user" $BUILDER_CONTAINER streamed
    docker run --rm --net none -v $PROJECTROOT/workdir/user:/home/user --workdir="/home/user" "$DOCKER_REGISTRY_HOST/$(./hash-files.sh)"
    exit 0
    ;;
  prune)
    get_image
    if [[ -n $(docker images -q "$ENV_IMAGE") ]]; then
      log "Removing image $ENV_IMAGE..."
